  ...
  async function processTicket(qrCodeData: string) {
    ...
    if (ticket) {
      ...
      if (ticket.status !== "reserved") {
        setScanResult({ success: false, message: `Deja procesat (${statusLabels[ticket.status]}).`, studentName: reservation.profiles?.display_name || `${reservation.profiles?.last_name} ${reservation.profiles?.first_name}`, ticketId: ticket.id });
        return;
      }

  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-xl sm:text-2xl font-bold">Raport pe zile</h1>
      ...
      <Input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full sm:w-56" />
            ...
            {events.map((e) => (
              <div key={e.id} className="rounded-lg border bg-card p-3 space-y-1">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className="text-xs text-muted-foreground shrink-0">{e.enrolled}/{e.max_capacity}</span>
                </div>
                <p className="text-xs text-muted-foreground">{e.start_time?.slice(0, 5)} - {e.end_time?.slice(0, 5)} · {e.counted_duration_hours}h</p>

...
export default function StudentReportPage() {
  ...
  const handleExport = () => {
    if (!report) return;
    const name = report.profile?`${report.profile?.last_name} ${report.profile?.first_name}`;
    exportReportPdf({
      ...
      {report && (
        <>
          <div className="grid gap-4 md:grid-cols-5">
            <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">Elev</CardTitle></CardHeader><CardContent><p className="text-lg font-bold">{report.profile?`${report.profile?.last_name} ${report.profile?.first_name}`}</p></CardContent></Card>
